...
const AVAILABLE_MONTHS = [...new Set(MONTHS)].sort();
const YEARS = [...new Set(AVAILABLE_MONTHS.map(m => m.slice(0, 4)))];

function combineAnnualLedgers(months, ledgers) {
  const warnings = ledgers.flatMap((d, i) => (d.warnings || []).map(w => `${monthLabel(months[i])}: ${w}`));
  const monthly = months.map((m, i) => {
    const d = ledgers[i];
    const income = sum(d.entries.filter(r => r.kind === 'income'));
    const expense = sum(d.entries.filter(r => r.kind === 'expense'));
    const closing = d.opening == null ? null : d.opening + income - expense;
    if (closing != null && d.reportedBalance != null && closing !== d.reportedBalance)
      warnings.push(`${monthLabel(m)}: 계산 잔액과 시트 잔액이 일치하지 않습니다.`);
    return { month: m, income, expense, opening: d.opening ?? null, closing, reportedBalance: d.reportedBalance ?? null };
  });
  monthly.forEach((row, i) => {
    const prev = monthly[i - 1];
    if (prev && Number(row.month.slice(5)) === Number(prev.month.slice(5)) + 1 && prev.closing != null && row.opening != null && prev.closing !== row.opening)
      warnings.push(`${monthLabel(row.month)}: 이월금이 전월 계산 잔액과 일치하지 않습니다.`);
  });
  const contiguous = months.length > 0 && months.every((m, i) => Number(m.slice(5)) === i + 1);
  return {
    ...emptyLedger,
    entries: ledgers.flatMap((d, i) => d.entries.map((r, j) => ({ ...r, id: `${months[i]}:${r.id}:${j}`, sourceMonth: months[i] }))),
    opening: contiguous ? ledgers[0].opening ?? null : null,
    reportedBalance: contiguous ? ledgers.at(-1).reportedBalance ?? null : null,
    warnings, monthly, sourceStatus: ledgers.every(d => d.sourceStatus === 'ok') ? 'ok' : 'error',
...
const NAV = [
  ...
  { id: 'report', label: '재정 결산', icon: ChartNoAxesCombined },
...
export default function App() {
  ...
  const [month, setMonth] = useState(AVAILABLE_MONTHS.at(-1));
  const [periodMode, setPeriodMode] = useState('month');
  const [year, setYear] = useState(AVAILABLE_MONTHS.at(-1).slice(0, 4));
  const annual = periodMode === 'year';
  const periodKey = annual ? year : month;
  const periodLabel = annual ? `${year}년` : monthLabel(month);
  const periodMonths = useMemo(() => annual ? AVAILABLE_MONTHS.filter(m => m.startsWith(`${year}-`)) : [month], [annual, year, month]);
  const missingMonths = annual ? Array.from({length: 12}, (_, i) => `${year}-${String(i + 1).padStart(2, '0')}`).filter(m => !periodMonths.includes(m)) : [];
  const coverage = annual ? `집계 대상: ${periodMonths.map(m => Number(m.slice(5)) + '월').join(', ')} (${periodMonths.length}/12개월)` : periodLabel;
  function changeMode(mode) { setPeriodMode(mode); if (mode === 'year') setYear(month.slice(0, 4)); else setMonth(AVAILABLE_MONTHS.filter(m => m.startsWith(year)).at(-1)); resetFilters(); }
  function changePeriod(value) { if (annual) setYear(value); else setMonth(value); resetFilters(); }
  const periodOptions = annual ? YEARS : AVAILABLE_MONTHS;
  const periodIndex = periodOptions.indexOf(periodKey);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => { timedOut = true; controller.abort(); }, annual ? 60000 : 20000);
    async function loadPeriod() {
      const results = [];
      // Three requests at a time limits load on the published Google Sheets.
      for (let i = 0; i < periodMonths.length; i += 3) {
        const batch = await Promise.all(periodMonths.slice(i, i + 3).map(async m => {
          try { return demo ? sampleLedger(m) : await fetchLedger(m, controller.signal); }
          catch (e) { throw new Error(`${monthLabel(m)}: ${e.message || '장부를 불러오지 못했습니다.'}`); }
        }));
        if (!active) return;
        results.push(...batch);
      }
      if (active) setLedger(annual ? combineAnnualLedgers(periodMonths, results) : results[0]);
    }
    loadPeriod().catch(e => { controller.abort(); if (active) setError(timedOut ? '시트 응답이 지연되고 있습니다. 다시 시도해 주세요.' : `${e.message} 전체 기간의 집계를 완료하지 못했습니다.`); }).finally(() => { clearTimeout(timer); if (active) setLoading(false); });
    return () => { active = false; clearTimeout(timer); controller.abort(); };
  }, [periodMonths, annual, demo, revision]);
  useEffect(() => { setPage(1); setSelected(null); setPerson(null); }, [search, category, payment, weekFilter, month, year, periodMode, view, order, demo]);
  ...
  const chartData = annual ? Array.from({length: 12}, (_, i) => { const row = ledger.monthly?.find(r => Number(r.month.slice(5)) === i + 1); return {name: `${i + 1}월`, income: row?.income ?? null, expense: row?.expense ?? null}; }) : Array.from({ length: Math.ceil(new Date(Number(month.slice(0, 4)), Number(month.slice(5)), 0).getDate() / 7) }, (_, i) => ({ name: `${i + 1}주차`, income: sum(incomes.filter(r => week(r.date) === i + 1)), expense: sum(expenses.filter(r => week(r.date) === i + 1)) }));
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter(r => (!search || r.name.includes(search) || r.note.includes(search)) && (category === '전체' || r.category === category) && (payment === '전체' || r.payment === payment) && (annual || weekFilter === '전체' || `${week(r.date)}주차` === weekFilter)).sort((a, b) => order === '금액 높은순' ? b.amount - a.amount : order === '오래된순' ? a.date.localeCompare(b.date) : b.date.localeCompare(a.date));
  ...
  function exportRows(rows = filtered) {
    ...
    const blob = new Blob([csvExport(exportMatrix(rows))], { type: 'text/csv;charset=utf-8' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `${demo ? '예시_' : ''}${periodKey}_${titleOf(view)}.csv`; a.click(); setTimeout(() => URL.revokeObjectURL(url), 5000); setFeedback('CSV 파일을 내려받았습니다.');
  }
  async function exportExcel() {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(book, XLSX.utils.aoa_to_sheet([['운정그리스도의교회', periodLabel], ['자료 구분', demo ? '예시 데이터' : '구글 시트'], ['집계 범위', coverage], [annual ? '연초 이월금' : '전월 이월금', ledger.opening ?? '미확인'], ['헌금·수입', totalIncome], ['지출', totalExpense], ['계산 잔액', closing ?? '미확인'], ['시트 잔액', ledger.reportedBalance ?? '미확인'], ['잔액 차이', difference ?? '미확인']]), annual ? '연간 결산' : '월간 결산');
      if (annual) XLSX.utils.book_append_sheet(book, XLSX.utils.aoa_to_sheet([['월', '헌금·수입', '지출', '수지 차액', '계산 잔액'], ...chartData.map((r, i) => [r.name, r.income ?? '자료 없음', r.expense ?? '자료 없음', r.income == null ? '자료 없음' : r.income - r.expense, ledger.monthly?.find(m => Number(m.month.slice(5)) === i + 1)?.closing ?? '미확인'])]), '월별 집계');
      ...
      XLSX.writeFile(book, `${demo ? '예시_' : ''}${periodKey}_교회재정결산.xlsx`); setFeedback('엑셀 결산 파일을 내려받았습니다.');
  ...
  return <div className="finance-root"><aside className="desktop-sidebar">{navContent()}</aside><div className="app-shell"><header className="topbar"><div className="breadcrumb"><button aria-label="메뉴 열기" className="icon-button mobile-menu" onClick={() => setMobileOpen(true)}><Menu size={20} /></button><span>재정 관리</span><ChevronRight size={14} /><strong>{titleOf(view)}</strong></div><div className="topbar-actions"><button className="privacy-button" onClick={mask} aria-pressed={hideNames}>{hideNames ? <EyeOff size={17} /> : <Eye size={17} />}<span>{hideNames ? '이름 가림' : '이름 표시'}</span></button><div className="avatar">재정</div></div></header>
    <main className="workspace"><div className="page-heading"><div><div className="eyebrow">UNJEONG CHURCH · FINANCE</div><h1>{titleOf(view)}</h1><p>{annual && view !== 'connection' ? `${year}년의 연결된 월별 장부를 합산해 확인합니다.` : DESCRIPTIONS[view]}</p></div><div className="heading-actions">{view === 'report' ? <button className="button" disabled={!ready} onClick={() => window.print()}><Printer size={16} />결산 인쇄</button> : <button className="button" disabled={!ready || !data.length} onClick={() => exportRows(view === 'overview' || view === 'connection' ? data : filtered)}><Download size={16} />CSV 내보내기</button>}<button className="button primary" disabled={loading} onClick={refresh}><RefreshCw size={16} className={loading ? 'spinning' : ''} />새로고침</button></div></div>
      <div className="period-bar" style={{flexWrap:'wrap', gap:12}}><div className="month-nav" style={{flexWrap:'wrap', gap:8}}>
        <Choice label="통계 단위" value={periodMode} onChange={changeMode} options={[{value:'month',label:'월별 통계'},{value:'year',label:'연간 통계'}]} />
        <button className="icon-button" aria-label={annual ? '이전 연도' : '이전 월'} disabled={periodIndex <= 0} onClick={() => changePeriod(periodOptions[periodIndex - 1])}><ChevronLeft size={18}/></button>
        <CalendarDays size={18}/><Choice label={annual ? '조회 연도' : '조회 월'} className="month-select" value={periodKey} onChange={changePeriod} options={periodOptions.map(p => ({value:p,label:annual ? `${p}년` : monthLabel(p)}))}/>
        <button className="icon-button" aria-label={annual ? '다음 연도' : '다음 월'} disabled={periodIndex >= periodOptions.length - 1} onClick={() => changePeriod(periodOptions[periodIndex + 1])}><ChevronRight size={18}/></button>
      </div><span className="sync-status" role="status">{demo ? '예시 데이터' : loading ? '장부 불러오는 중…' : error ? '연결 확인 필요' : '구글 시트 연동'}</span></div>
      {annual && <div className="notice"><Info size={18}/><span><b>{coverage}</b>{missingMonths.length > 0 && <><br/>자료 없는 달: {missingMonths.map(m => Number(m.slice(5)) + '월').join(', ')} · 표시 금액은 연결된 달의 누계입니다.</>}</span></div>}
    ...
    <Modal open={!!person} onClose={() => setPerson(null)} title={`${person ? name(person) : ''} 헌금 내역`}><div className="detail-body"><p className="muted">{periodLabel} · 이름 기준 합산</p><div className="detail-amount">{won(sum(incomes.filter(r => r.name === person && r.category !== '기타수입')))}<small>원</small></div>{incomes.filter(r => r.name === person && r.category !== '기타수입').sort((a, b) => b.date.localeCompare(a.date)).map(r => <div className="person-detail-row" key={r.id}><div><b>{r.category}</b><span>{r.date} · {r.payment}</span></div><strong>{won(r.amount)}원</strong></div>)}</div></Modal>
